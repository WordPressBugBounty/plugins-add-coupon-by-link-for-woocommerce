...
v1.0.1
*/
if(!class_exists('pisol_url_coupon_review')){

class pisol_url_coupon_review{
    ...
    function display_admin_notice() {
        ...
        $notice .= '<p>'.sprintf(__("Hi there, You've been using <strong>%s</strong> on your site for a few days <br>- I hope it's been helpful. If you're enjoying my plugin, would you mind rating it 5-stars to help spread the word?",'add-coupon-by-link-woocommerce'), $this->title).'</p>';
        $notice .= '<ul class="pi-flex" style="margin-top:15px;
        grid-template-columns: 1fr 1fr 1fr;
        grid-column-gap: 20px;
        text-align: center;">';
        $notice .= '<li><a val="later" class="pi-active-btn pisol-review-btn" href="'.add_query_arg(array('action' => "pi_save_review_preference_{$this->slug}", 'preference'=>'later',  '_wpnonce'=>wp_create_nonce( "pi_save_review_preference_{$this->slug}" )), admin_url('admin-post.php')).'">'.__("Remind me later",'add-coupon-by-link-woocommerce').'</a></li>';
        $notice .= '<li><a  class="pi-active-btn pisol-review-btn" style="font-weight:bold;" val="given" href="'.add_query_arg(array('action' => "pi_save_review_preference_{$this->slug}", 'preference'=>'now','_wpnonce'=>wp_create_nonce( "pi_save_review_preference_{$this->slug}" )), admin_url('admin-post.php')).'" target="_blank">'.__("Review Here",'add-coupon-by-link-woocommerce').'</a></li>';
        $notice .= '<li><a  class="pi-passive-btn pisol-review-btn" val="never" href="'.add_query_arg(array('action' => "pi_save_review_preference_{$this->slug}", 'preference'=>'never', '_wpnonce'=>wp_create_nonce( "pi_save_review_preference_{$this->slug}" )), admin_url('admin-post.php')).'">'.__("I would not",'add-coupon-by-link-woocommerce').'</a></li>';	 
        if($this->buy_url && $this->price){   
            /* translators: Price */    
            $notice .= '<li><a target="_blank" class="pi-buy-now-btn pisol-review-btn" val="never" href="'.esc_url($this->buy_url).'&utm_ref=review_reminder">'.sprintf(__("BUY PRO FOR %s",'add-coupon-by-link-woocommerce'), $this->price).'</a></li>';	
        }        
        $notice .= '</ul>';
        $notice .= '</div>';
        $notice .= '</div>';
...
new pisol_url_coupon_review('Add coupon by link plugin', 'add-coupon-by-link-for-woocommerce');
}