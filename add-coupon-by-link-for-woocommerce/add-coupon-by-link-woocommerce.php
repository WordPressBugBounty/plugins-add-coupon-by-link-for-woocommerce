...
if(is_plugin_active( 'add-coupon-by-link-for-woocommerce-pro/add-coupon-by-link-for-woocommerce-pro.php') ){
 ...
 * Start at version 1.2.16 and use SemVer - https://semver.org
 * Rename this for your plugin and update it as you release new versions.
 */
define( 'ADD_COUPON_BY_LINK_WOOCOMMERCE_VERSION', '1.2.16' );
define( 'PISOL_ACLW_VERSION', '1.2.16' );
define( 'PISOL_ACBLW_PRICE', '$25' );
...
function activate_add_coupon_by_link_woocommerce() {
    ...
    add_option('pi_acblw_do_activation_redirect', true);
    add_option('pi_'.dirname( plugin_basename( __FILE__ ) ), ADD_COUPON_BY_LINK_WOOCOMMERCE_VERSION);
}
...
add_action('admin_init', function (){
    if (get_option('pi_acblw_do_activation_redirect', false)) {
        delete_option('pi_acblw_do_activation_redirect');
        if(!isset($_GET['activate-multi']))
        {
            wp_redirect("admin.php?page=pi-acblw-coupon&tab=default");
        }
    }
});

add_action('plugins_loaded', function () {
    $stored_version = get_option('pi_'.dirname( plugin_basename( __FILE__ ) ).'_version', '');

    if ( $stored_version !== ADD_COUPON_BY_LINK_WOOCOMMERCE_VERSION ) {
        update_option('pi_'.dirname( plugin_basename( __FILE__ ) ).'_version', ADD_COUPON_BY_LINK_WOOCOMMERCE_VERSION);
    }
});
