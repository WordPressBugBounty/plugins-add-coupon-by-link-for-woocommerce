...
class pisol_acblw_woo_payment_block{

    private $extend;

    protected static $instance = null;

    const IDENTIFIER = 'pisol_acblw_set_payment_method';
    ...
    function paymentScript(){
        if(is_checkout()){
            wp_enqueue_style( 'pisol-acblw-payment-block', plugin_dir_url( __FILE__ ) . 'css/block.css', array(  ), '1.0.0');
            wp_enqueue_script( 'pisol-acblw-payment-block', plugin_dir_url( __FILE__ ) . 'js/block-payment.js', array( 'wp-plugins', 'wc-blocks-checkout' ), '1.0.0', true );
        }
    }
