...
class pisol_acblw_apply_coupon{
    function __construct(){
        ...
        add_action( 'woocommerce_applied_coupon', [$this, 'auto_add_product_when_coupon_applied'], 10, 1 );
    }
    ...
    function applyUrlCoupon(){
        ...
        if ( isset( $_GET[ $key ] ) && '' !== $_GET[ $key ] && function_exists( 'WC' ) ) {  
            ...
            /** will remove the apply_coupon from the url once the coupon is applied that way url changes and user will not go on adding new product to cart if there is auto add product present in the cart */
            if(apply_filters('pisol_acblw_redirect_to_remove_coupon', false) && self::is_there_auto_add_product($_GET[ $key ])){
    ...
    function is_there_auto_add_product($coupon_code){
    ...
    function applyCoupon( $coupon_code ) {
        ...
        do_action( 'pisol_acblw_before_coupon_applied', $coupon_code);
            ...
            if ( $the_coupon->is_valid() && ! WC()->cart->has_discount( $coupon_code )) {
                ...
                if($result){
                    self::removeCouponFromSession( $coupon_code );
                }
            }else{
    ...
    function auto_add_product_when_coupon_applied($coupon_code){
        $coupon = new WC_Coupon( $coupon_code );
        
        if(is_object($coupon) && $coupon->get_id() != 0){
            $product_ids = $this->get_auto_apply_products($coupon);
            if(!empty($product_ids) && is_array($product_ids)){
    ...
    function get_auto_apply_products($coupon){
        $product_ids = $coupon->get_meta( 'pisol_auto_add_products_apply' );
        if(!empty($product_ids) && is_array($product_ids)){
            return apply_filters('pisol_acblw_auto_apply_products', $product_ids, $coupon);
        }
        return apply_filters('pisol_acblw_auto_apply_products', array(), $coupon);
    }

    static function removeCouponFromSession( $coupon_code ){
        $coupons = WC()->session->get( 'pisol_url_coupons', array() );
        if (is_array($coupons) && ( $key = array_search( $coupon_code, $coupons ) ) !== false ) {
            unset( $coupons[ $key ] );
            WC()->session->set( 'pisol_url_coupons', $coupons );
        }
    }
