...
class StoreCredit {
    ...
    static function get_original_amount($coupon_id){
        if(is_object($coupon_id) && get_class($coupon_id) == 'WC_Coupon'){
            $coupon = $coupon_id;
        }else{
            $coupon = new \WC_Coupon($coupon_id);
        }
        
        remove_filter('woocommerce_coupon_get_amount', [ __CLASS__,'getAmount' ], 10, 2 );
