...
class pisol_acblw_qr_code{
    ...
    function meta_box($post){
        ...
        if($post_id){
			...
			printf('<div style="text-align:center; margin-top:20px" ><a href="javascript:void(0)" id="pi-qr-code-download" style="display:none;" class="button">'.esc_html__('Download Qr Code','add-coupon-by-link-woocommerce').'</a></div>');
		}else{
            echo '<p>'.esc_html__('Save coupon then QR code will be generated','add-coupon-by-link-woocommerce').'</p>';
        }
    }
    ...
    function enqueue_admin_script(){
        ...
        if (is_object($screen) && $screen->id == 'shop_coupon') {
            ...
            wp_localize_script($this->plugin_name."_coupon_url", 'wc_product_search_params', [
