...
class Analytics{
    ...
    public function show_tracker_notice() {
        ...
        $notice .= '<p>'.__("Hi, your support can make a big difference!", 'add-coupon-by-link-woocommerce').'</p>';
        $notice .= '<p>'.__("We collect only technical data — including the plugin version, WordPress version, WooCommerce version, and site url — solely to improve compatibility and enhance plugin features.", 'add-coupon-by-link-woocommerce').'</p>';
        ...
        $notice .= sprintf(
            ...
            __('I Don\'t Help', 'add-coupon-by-link-woocommerce')
        );
        $notice .= sprintf(
            ...
            __('I Will Help', 'add-coupon-by-link-woocommerce')
        );
    ...
    public function handle_deactivation_form() {
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'pi_deactivate_nonce_' . $this->plugin_slug)) {
            wp_die(esc_html__('Security check failed', 'add-coupon-by-link-woocommerce'));
        }
        ...
        if (is_multisite() && is_plugin_active_for_network($this->plugin_path)) {
            if (is_super_admin()) {
                ...
                wp_die(esc_html__('You do not have permission to deactivate a network plugin.', 'add-coupon-by-link-woocommerce'));
            }
        } else {
...
new Analytics(
            'WooCommerce Add coupon by link',
            'add-coupon-by-link-for-woocommerce/add-coupon-by-link-woocommerce.php',
